...
protein_water = BSS.IO.readMolecules(BSS.IO.glob("%s/../2HHN_aligned_to_chain_B_5qc6_TER_water.pdb" % job_dir)).getMolecules()[0]
...
lig0 = BSS.IO.readMolecules(BSS.IO.glob("%s/../aligned/parameterised/CatS_%s.*" % (job_dir, num0))).getMolecules()[0]
lig1 = BSS.IO.readMolecules(BSS.IO.glob("%s/../aligned/parameterised/CatS_%s.*" % (job_dir, num1))).getMolecules()[0]
...
freenrg = BSS.FreeEnergy.Binding(solvated, protocol, work_dir="CatS_%s_%s" % (num0, num1), engine="SOMD", box=3*[30*BSS.Units.Length.angstrom])
